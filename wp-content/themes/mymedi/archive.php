...
$show_breadcrumb = apply_filters('mymedi_show_breadcrumb_on_archive_post', true);
$show_page_title = apply_filters('mymedi_show_page_title_on_archive_post', true);
$page_title = '';
...
if( $show_page_title ){
	switch( true ){
		case is_day():
			$page_title = esc_html__( 'Day: ', 'mymedi' ) . get_the_date();
		break;
		case is_month():
			$page_title = esc_html__( 'Month: ', 'mymedi' ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'mymedi' ) );
		break;
		case is_year():
			$page_title = esc_html__( 'Year: ', 'mymedi' ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'mymedi' ) );
		break;
		case is_search():
			$page_title = esc_html__( 'Search Results for: ', 'mymedi' ) . get_search_query();
		break;
		case is_tag():
			$page_title = esc_html__( 'Tag: ', 'mymedi' ) . single_tag_title( '', false );
		break;
		case is_category():
			$page_title = esc_html__( 'Category: ', 'mymedi' ) . single_cat_title( '', false );
		break;
		case is_404():
			$page_title = esc_html__( 'OOPS! FILE NOT FOUND', 'mymedi' );
		break;
		default:
			$page_title = esc_html__( 'Archives', 'mymedi' );
		break;
	}
}

mymedi_breadcrumbs_title($show_breadcrumb, $show_page_title, $page_title);
?>
