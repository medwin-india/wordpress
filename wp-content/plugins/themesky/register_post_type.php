...
if( !class_exists('TS_Portfolios') ){
	class TS_Portfolios{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(393, 300);
			$this->add_image_size();
...
if( !class_exists('TS_Footer_Blocks') ){
	class TS_Footer_Blocks{
		...
		function add_custom_css(){
			if( !function_exists('mymedi_get_theme_options') ){
				return;
			}
			$custom_css = '';
			
			$first_footer_area = mymedi_get_theme_options('ts_first_footer_area');
			$second_footer_area = mymedi_get_theme_options('ts_second_footer_area');
